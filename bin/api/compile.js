...
(function() {
  var coffeescript, exec, fs, jade, less, livescript, path, sass, stylus, _;
  ...
  exports['compass'] = function(input, back) {
    var args, err, option, options, output, value, _i, _len, _ref;
    if (!_.isFunction(back)) {
      throw new Error('callback is required {function}.');
    }
    if (!_.isObject(input)) {
    ...
    try {
      ...
      output.source_map = null;
      output.warnings = [];
      output.warnings.push('compass is still not available.');
      options = {};
      options.cwd = path.resolve(process.cwd());
      args = ['config'];
      args.push(options.cwd + '\\' + 'compass.rb');
      _ref = _.keys(input.config.compass.options);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        option = _ref[_i];
        value = input.config.compass.options[option];
        if (!_.isBoolean(value)) {
          args.push('--' + _.string.dasherize(option + '=' + value));
        } else if (value === true) {
          args.push('--' + _.string.dasherize(option));
        }
      }
      console.log(args);
      return exec('compass.bat', args, function(err, out, code) {
        if (err) {
          return back(err);
        }
        return back(null, output);
  ...
  exports['sass'] = function(input, back) {
    ...
    try {
      if (input.config.compass.enabled === true) {
        return this.compass(input, back);
      }
      if (_.isEmpty(input.code)) {
        input.code = fs.readFileSync(input.file, 'utf8');
      }
